...
export function remarkCodeTabs() {
  return (tree) => {
        ...
        if (node.type === "code") {
          ...
          if (currentGroup.length > 0) {
            ...
            for (let j = lastCodeBlockIndex + 1; j < i; j++) {
              ...
              const isEmpty =
                (between.type === "paragraph" &&
                  (!between.children || between.children.length === 0)) ||
                (between.type === "text" &&
                  (!between.value || between.value.trim() === ""));
              if (!isEmpty) {
                isConsecutive = false;
                break;
              }
    ...
    for (const group of groups.reverse()) {
      ...
      const tabButtonsHtml = tabLabels
        .map(
          (label, i) =>
            `<h3 class="code-tab-btn${i === 0 ? " active" : ""}" data-tab-index="${i}">${label}</h3>`,
        )
        .join("");
      ...
      const openingHtml = {
        type: "html",
        value: `<div class="code-tabs"><div class="code-tabs-header">${tabButtonsHtml}</div><div class="code-tabs-content">`,
      };

      // Wrap each code block in a panel
      const wrappedBlocks = group
        .map(({ node }, i) => [
          {
            type: "html",

...
If you want to convert the screen space position to world space and save it to the object's `transform` properties, you can use `syncFromDom(stage)`:
```typescript
object.queueUpdate("READ_1", () => {
    object.readDom(); // Reads DOM and caches to READ_1 slot
    object.syncFromDom("READ_1"); // Converts screen position to world position and saves to transform properties
});
```

`readDom()` can also take an optional boolean parameter `subtractAppliedTransform` (default: `false`).
...
`requestRead(subtractAppliedTransform?, syncToWorldPosition?, stage?, recursive?)` essentially combines
`readDom(subtractAppliedTransform)`, `syncFromDom()` and `queueUpdate()` into a single method call.
```typescript

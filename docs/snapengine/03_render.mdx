---
title: Render Queue
description: Learn how to efficiently read and write to the DOM using SnapEngine's queuing system.
order: 3
...
But we already mentioned in the previous section that DOM reads can be expensive. 
On top of that, writes can be expensive too. Even worse, firing many reads and writes one after another can cause a phenomenon 
known as **layout thrashing**. This occurs when you read something from the DOM after modifying it
...
- `relative`:   Applies the object's transform properties such that it will be positioned on screen where it is specified by the transform properties,
...
- `offset`:     Similar to `relative`, but instead of calculating the final position based on the current object's DOM properties, 
                it uses the _transform_ properties of a designated `transformOrigin` object. This is useful for creating parent-child relationships where the child object's position is relative to the parent object.

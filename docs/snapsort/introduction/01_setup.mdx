...
Items can now be dragged to reorder, with FLIP animations included by default. The list on screen updates, but your `tasks` array does not — wiring drops back into framework state is covered in [State-Backed Lists](/docs/snapsort/guides/03_state_backed_lists).
